...
#Iterating to find average performance
...
features_number = kernel.capitalize() + " Kernel, " + str(len(features_index)) + " Features"
...
for i in range(1000):
    ...
    class_error_collected.append(class_error)
    class_error = []
    training_set_size = []


class_error_average = [ [] for i in range(len(training_set_size))]

for i in range(len(class_error_collected)):
    for j in range(len(training_set_size)):
        class_error_average[j].append(class_error_collected[i][j])

class_error_average_calc = [ 0 for i in range(len(class_error_average))]

for i in range(len(class_error_average)):
    class_error_average_calc[i] = sum(class_error_average)/float(len(class_error_average))
    

plt.scatter(training_set_size, class_error_average, s=10, marker="x", label=features_number)
plt.xlabel("Number of samples in training set")
plt.ylabel("Classification Accuracy (%)")
plt.ylim([0,100])
plt.title("Average accuracy of SVM classifier with different training set sizes \nfor a fixed test set size")
plt.legend(loc=2, ncol=3, borderaxespad=2)
plt.show()
