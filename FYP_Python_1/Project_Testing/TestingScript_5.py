#Testing effect of number of features on performance, with proper feature scaling
#Testing the performance of a classifier induced from raw data
#Fixed 0.48/0.52 test set/training set split
...
C = 20
normalization = True
test_size = 0.48 #Has to be a multiple of 0.04
test_steps = int(((1 - test_size) / 0.04) + 1)

#Adjust to drop different classes from DF
sample_drop_index1 = [ i for i in range(0,0)] 
...
#Creating list of lists of all possible feature combinations

features_indexes = []
subset_list = []

for L in range(0, len(features_index)+1):
    for subset in itertools.combinations(features_index, L):
        subset_list = []
        for i in range(len(subset)):
            subset_list.append(subset[i])
            features_indexes.append(subset_list)
...
for features_index in features_indexes:
    ...
    num_features.append(len(features_index))

plt.scatter(num_features, class_error, s=10, marker="x")
plt.title("Effect of number of features on classifier performance")
plt.xlabel("Number of features used")
plt.ylabel("Classification accuracy on partitioned test set (%)")
plt.show()
