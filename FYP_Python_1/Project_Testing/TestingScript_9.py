...
for C in range(1, 100, 2):
    ...
    X_predicted = svm.predict(X_test)
    
    counter = 0

    for sample_number in range(len(X_predicted)):
        if X_predicted[sample_number] != y_test[sample_number]:
            counter += 1
...
plt.scatter(C_list, class_error, s=10, marker="x", label=features_number)
plt.xlabel("Value of C")
plt.ylabel("Classification Accuracy (%)")
plt.ylim([58,102])
plt.title("Accuracy of SVM classifier with different C values, for a fixed testing \nand training set size")
