...
plt.scatter(training_set_size, class_error, s=10, marker="x", label=features_number)
plt.xlabel("Number of samples in training set")
plt.ylabel("Classification Accuracy (%)")
plt.ylim([58,102])
plt.title("Accuracy of SVM classifier with different training set sizes \nfor a fixed test set size")
plt.legend(loc=7, ncol=3, borderaxespad=2)
plt.show()
