...
while k < num_samples:
    ...
    if IMUPacket.hex() == start_bytes:
        ...
        xgyroupdate = twos_comp(byte_data_whole[k,2] | (byte_data_whole[k,3] << 8), 16)
        ygyroupdate = twos_comp(byte_data_whole[k,4] | (byte_data_whole[k,5] << 8), 16)
        zgyroupdate = twos_comp(byte_data_whole[k,6] | (byte_data_whole[k,7] << 8), 16)
        xgyro.append(xgyroupdate)
        ygyro.append(ygyroupdate)
        zgyro.append(zgyroupdate)
        lastbyte.append(byte_data_whole[k,36])
