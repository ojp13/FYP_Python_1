...
threshold = 3500    # Value above which we believe a movement to be happening in the acceleration data
...
move_acc_array = np.zeros([stepValue + backTrack, n, 3], dtype = float)
...
#moveData = {}   # Dictionary containing data about each movement

#for i in range(1, numMoves + 1):
#    name = "move" + str(i)
#    moveData[name] = [accXSmoothed[(indexValues[i-1]-backTrack):(indexValues[i-1]+stepValue)],
#                      accYSmoothed[(indexValues[i-1]-backTrack):(indexValues[i-1]+stepValue)],
#                      accZSmoothed[(indexValues[i-1]-backTrack):(indexValues[i-1]+stepValue)]]

#for name in moveData.keys():
#    print(name)

#print(moveData['move1'])

#plt.scatter(range(0,len(moveData['move1'][0])), moveData['move1'][0][:], s = 0.1)
#plt.scatter(range(0,len(moveData['move1'][0])), moveData['move1'][1][:], s = 0.1)
#plt.scatter(range(0,len(moveData['move1'][0])), moveData['move1'][2][:], s = 0.1)
